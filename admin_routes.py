...
def create_contract():
    ...
    contract_number = f"CTR-{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def confirm_payment(payment_id):
    # บันทึกเวลาชำระเงินตามเวลาจริงในประเทศไทย
    ...
    with get_db() as conn:
        ...
        if payment:
            receipt_no = f"REC-{payment['contract_id']}-{payment['installment_no']}-{datetime.datetime.now().strftime('%M%S')}"
            cursor.execute("""
                UPDATE payments 
...
def close_contract_early(contract_id):
    """ฟังก์ชั่นให้ Admin อนุมัติการปิดยอดของลูกค้า (ส่วนลด 15%)"""
    now_str = datetime.datetime.now(TH_TZ).strftime('%Y-%m-%d %H:%M:%S')
    ...
    with get_db() as conn:
        ...
        if contract:
            ...
            for p in unpaid_payments:
                receipt_no = f"REC-EARLY-{contract_id}-{p['installment_no']}-{datetime.datetime.now().strftime('%M%S')}"
                cursor.execute("""
                    UPDATE payments 
                    SET status = 'paid', paid_at = ?, receipt_no = ?
                    WHERE id = ?
